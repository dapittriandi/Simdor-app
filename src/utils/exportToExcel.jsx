...
const formatDate = (timestamp) => {
  ...
  if (timestamp.seconds) {
    ...
    if (!isNaN(date.getTime())) {
      return date; // Return as Date object for XLSX
    }
  }

  // Handle potential string or number inputs representing dates
  try {
  ...
  // If it's not a recognized format or invalid, return null or a placeholder
  // Returning null is better for XLSX if you want empty cells
  return null;
  // Alternatively, return a placeholder string: return "-";
};
...
export const exportToExcel = (orders, fileName = "Laporan Orders") => {
  ...
  const data = orders.map((order) => {
      ...
      return [
          ...
          getDateCell(order.distribusiSertifikatPengirimTanggal),
          getDateCell(order.distribusiSertifikatPenerimaTanggal)
      ];
  });
